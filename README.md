# Kevin, a simple KV-store as an unikernel with HTTP endpoints

Kevin is a simple unikernel that, from a block device, has a Key-Value store
...
```shell
$ git clone https://git.robur.coop/robur/kevin.git
$ cd kevin
...
$ solo5-hvt --net:service=tap0 --block:rowex=rowex.idx -- \
  kevin.hvt --ipv4=10.0.0.2/24 --color=always
```
